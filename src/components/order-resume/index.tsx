import React from 'react';
import Link from 'next/link';
import styles from './order-resume.module.scss';
import Title from '../title';
import CustomButton from '@/components/custom-button';
import { useTranslation } from 'react-i18next';
import { Skeleton } from '@mui/material';

export interface OrderResumeData {
  count: number;
  sub_total: number | string;
  shipping_cost: number | string | any;
  total: number | string;
  currency?: string;
}
export interface OrderResumeProps {
  data: OrderResumeData;
  hideShippingCost?: boolean;
  OnCheckout: () => void;
  loading: boolean;
  disabled?: boolean;
}

const OrderResume: React.FC<OrderResumeProps> = (props: OrderResumeProps) => {
  const { t } = useTranslation('cart');
  const { data } = props;
  return (
    <div className={styles.card_resume}>
      <Title title={t('summary')} className={styles.resume_title} />
      <div className={styles.content}>
        <div className={styles.resume_separated_info}>
          <p> {props.loading ? <Skeleton typeof='..........' width={100} height={30}/> : `${t('product')} (${data.count})` } </p>
          <h5> {props.loading ? <Skeleton typeof='..........' width={100} height={30}/> : data.sub_total } </h5>
        </div>
        <hr />
        {
          props.hideShippingCost ? null : (
            <>
              <div className={styles.resume_separated_info}>
                  <p>{props.loading ?  <Skeleton typeof='..........' width={100} height={30}/> : t('delivery') } </p>
                  <h5>{props.loading ? <Skeleton typeof='..........' width={100} height={30}/> : data.shipping_cost}</h5>
              </div>
              <hr />
            </>
            )
          }
          <div className={styles.resume_separated_info}>
            <p>{props.loading ? <Skeleton typeof='..........' width={100} height={30}/> : t('total')}</p>
            <h5>{props.loading ? <Skeleton typeof='..........' width={100} height={30}/> : data.total}</h5>
          </div>
          <hr />
      </div>

      <div className={styles.resume_buttons}>
        <CustomButton disabled={props.loading || props.disabled} backgroundColor ='#00C02A' textColor="#fff" onClick={props.OnCheckout}>
          <Link href="#">{t('checkout')}</Link>
        </CustomButton>
      </div>
    </div>
  );
}

export default OrderResume;
